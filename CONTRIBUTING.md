...
Thank you for considering to contribute to `semantic-release-plugin-github-breaking-version-tag` 💖
...
Node.js 12 or higher is required. Install it from https://nodejs.org/en/. [GitHub's `gh` CLI](https://cli.github.com/) is recommended for the initial setup

1. Fork this repository and clone it to your local machine. Using `gh` you can do this

   ```
   gh repo fork gr2m/semantic-release-plugin-github-breaking-version-tag
   ```

2. After cloning and changing into the `semantic-release-plugin-github-breaking-version-tag` directory, install dependencies and run the tests
...
Unless the change is trivial such as a type, please [open an issue first](https://github.com/gr2m/semantic-release-plugin-github-breaking-version-tag/issues/new) before starting a pull request for a bug fix or a new feature.
